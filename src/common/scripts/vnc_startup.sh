...
echo -e "\n------------------ start noVNC  ----------------------------"
if [[ $DEBUG == true ]]; then echo "$NO_VNC_HOME/utils/launch.sh --vnc localhost:$VNC_PORT --listen $NO_VNC_PORT --web ${WEB_ROOT:-./}"; fi
$NO_VNC_HOME/utils/launch.sh --vnc localhost:$VNC_PORT --listen $NO_VNC_PORT --web ${WEB_ROOT:-./} &> $STARTUPDIR/no_vnc_startup.log &
PID_SUB=$!
